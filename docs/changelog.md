...
- `limits.max_pages` + URL `{{page}}`：BT 来源可以多翻几页。Anime Garden 设为 3 页 × 100 条——排球少年第一季的整季 BDRip 在第 2 页，只看第一页全是剧场版和续作。此前客户端只拿到 `match.subjectTitle`（请求里的作品名），字幕组、季数、清晰度全丢，多季作品（排球少年四季）在客户端分不清哪一季。
