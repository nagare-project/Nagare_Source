...
状态：实验实现，2026-09-09。本次验证未启动 Nagare，客户端实际播放仍待验收。
...
2. 在 Nagare 中验收“选作品 → 点集 → mpv 起播”，继续验证首帧、拖动进度、完整播放及失败自动换源。HLS 还需验证分片和密钥请求。
3. 按来源健康度和优先级调度浏览器解析。当前最多两个会话，搜索仍并发，排队计入来源 deadline；全来源同时查找可能导致后排来源超时，需要独立处理调度预算。
